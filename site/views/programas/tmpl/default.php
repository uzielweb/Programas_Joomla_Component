...
<form action="" method="post" name="adminForm" id="adminForm">
	...
	<div class="row" id="programalist">
		...
		<div class="programlist span12 col-md-12">
            ...
            <?php
            $alldays = array(JText::_('COM_PROGRAMAS_PROGRAMAS_DAYS_OF_THE_WEEK_OPTION_SUNDAY'),JText::_('COM_PROGRAMAS_PROGRAMAS_DAYS_OF_THE_WEEK_OPTION_MONDAY'),JText::_('COM_PROGRAMAS_PROGRAMAS_DAYS_OF_THE_WEEK_OPTION_TUESDAY'),JText::_('COM_PROGRAMAS_PROGRAMAS_DAYS_OF_THE_WEEK_OPTION_WEDNESDAY'),JText::_('COM_PROGRAMAS_PROGRAMAS_DAYS_OF_THE_WEEK_OPTION_THURSDAY'),JText::_('COM_PROGRAMAS_PROGRAMAS_DAYS_OF_THE_WEEK_OPTION_FRIDAY'),JText::_('COM_PROGRAMAS_PROGRAMAS_DAYS_OF_THE_WEEK_OPTION_SATURDAY'));
            foreach ($alldays as $d => $oneday){
            ?>
               <div class="program">
                   <div>
                       <h3 class="oneday"><?php echo $oneday;?></h3>
            <?php
            foreach ($this->items as $i => $item)
                { ?>

                <?php if ($cat_title == $item->category){ ?>
            ...
            <?php
                ...
                <?php if (isset($this->items[0]->state)) : ?>
                    ...
                    <div class="center">
                        <a class="btn btn-micro <?php echo $class; ?>" href="<?php echo ($canChange) ? JRoute::_('index.php?option=com_programas&task=programa.publish&id=' . $item->id . '&state=' . (($item->state + 1) % 2), false, 2) : '#'; ?>">
                            <?php if ($item->state == 1): ?>
                                <i class="<?php echo $iconfont;?>-publish"></i>
                            <?php else: ?>
                                <i class="<?php echo $iconfont;?>-unpublish"></i>
                            <?php endif; ?>
                        </a>
                                        </div>
                <?php endif; ?>
                ...
                <div class="category">
                    <i class="<?php echo $iconfont;?>-flag"></i><?php echo $item->category; ?>

                </div>
                <div class="programas_genre">
                                <i class="<?php echo $iconfont;?>-music"></i><?php echo $item->genre; ?>
                </div>
                <div>
                                <i class="<?php echo $iconfont;?>-arrow-up"></i><?php echo JHtml::date($item->start_time, 'H:i'); ?> h
                                <br>
                                <i class="<?php echo $iconfont;?>-arrow-down"></i><?php echo JHtml::date($item->end_time, 'H:i'); ?> h
                </div>
                <div>
                                <i class="<?php echo $iconfont;?>-calendar"></i><?php echo $item->days_of_the_week; ?>
                </div>
                <div>

                                <?php if ($link_broadcaster == 1) : ?>
                                <a class="modal" href="<?php echo $item->broadcaster_image; ?>">
                                <?php endif; ?>
                                <i class="<?php echo $iconfont;?>-user"></i><?php echo $item->broadcaster_name; ?>
                                <?php if ($link_broadcaster == 1) : ?>
                                </a>
                                <?php endif; ?>
                </div>
                <div>
                                <i class="<?php echo $iconfont;?>-mail"></i><?php echo $item->broadcaster_email; ?>
                </div>
    ...
    $texts = explode($tag,$item->program_description);
                        ...
                        if ($counttext == '0' and $showintro == '1'){?>
                         ...
                         <?php if ($showreadmore == '1'){ ?>
                             ...
                             <a class="btn button button-alarm" href="<?php echo JRoute::_('index.php?option=com_programas&view=programa&id='.(int) $item->id); ?>">
                             <?php echo JText::_('COM_PROGRAMAS_READ_MORE');?>
                             </a>
                            <?php } ?>
                            </div>
                        <?php } ?>
                    <?php } ?>
                        <?php if ($canEdit || $canDelete): ?>
                            <div class="center">
                        <?php if ($canEdit): ?>
                            <a href="<?php echo JRoute::_('index.php?option=com_programas&task=programaform.edit&id=' . $item->id, false, 2); ?>" class="btn btn-mini" type="button"><i class="<?php echo $iconfont;?>-edit" ></i></a>
                        <?php endif; ?>
                        <?php if ($canDelete): ?>
                            <a href="<?php echo JRoute::_('index.php?option=com_programas&task=programaform.remove&id=' . $item->id, false, 2); ?>" class="btn btn-mini delete-button" type="button"><i class="<?php echo $iconfont;?>-trash" ></i></a>
                        <?php endif; ?>
                            </div>
                        <?php endif; ?>
	...
	<?php if ($canCreate) : ?>
		<a href="<?php echo JRoute::_('index.php?option=com_programas&task=programaform.edit&id=0', false, 2); ?>"
		   class="btn btn-success btn-small"><i
				class="<?php echo $iconfont;?>-plus"></i>
